...
with st.sidebar:
    ...
    st.title("🛡️ Engine Guardrails")
    st.info("System running: **microsoft/phi-2** (4-bit Quantized)")
            ...
            with st.spinner("Authenticating & Consulting Knowledge Base..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # Show Source Attribution for Explainability
                        if data.get("sources"):
                            with st.expander("🔍 Why this recommendation? (Explainability)"):
                                for src in data["sources"]:
                                    st.write(f"- Found in: **{src.get('name', 'Financial Product Database')}**")
...
with col2:
    ...
    st.metric("Model Size", "2.7B Params", "Quantized to 4-bit")
    st.metric("VRAM Usage", "1.6 GB", "Low-power optimized")
